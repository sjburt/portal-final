...
void portale(elapsedMillis t_milli) {
  int portal_distance = floor(fmodf(((float)t_milli / PORTAL_SPEED), PORTAL_SPACING));
  for (int i = 0; i < leds.len; i++) {
    uint8_t portal_no = NUM_STRIPS - (i/NUM_LEDS_PER_STRIP);
    if (portal_no == portal_distance) {
      leds[i] = CRGB::White;
    } else {
      randomSeed(i);
      leds[i] = CHSV(random8(), 255, 32);
    }
  }
}
...
void norm_sequence(elapsedMillis t_milli) {
  ...
  float rise_rate =  map((int)t_milli, 5000.0, 12000, 1, MAX_RR);
  switch (phase)
  {
  case RISE_IN:
  ...
  case SPARKS:
    ...
    sparks(map((int)t_milli, 12000, 20000, 200, 1000));
    break;
